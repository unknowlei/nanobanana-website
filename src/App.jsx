...
// 🟢 全球 CDN 图片加�?const getOptimizedUrl = (url, width = 400) => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-4 space-y-6">
            <div className="bg-white/70 backdrop-blur-md border border-white/50 rounded-3xl p-6 shadow-sm">
                ...
                <div className="space-y-4">
                    <div className="relative group"><input type="file" accept="image/*" multiple onChange={handleSourceUpload} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10"/><div className="border-2 border-dashed border-indigo-200 bg-indigo-50/30 rounded-2xl p-6 text-center hover:border-indigo-400 transition-all"><div className="flex flex-col items-center text-indigo-400"><Upload className="w-8 h-8 mb-2" /><span className="text-xs font-bold">上传大图 (支持多�?</span></div></div></div>
                    {sourceImages.length > 0 && (<div className="bg-white/50 rounded-xl border border-indigo-100 overflow-hidden"><div className="px-3 py-2 bg-indigo-50/50 border-b border-indigo-100 text-xs font-bold text-indigo-400 flex justify-between"><span>待处理列�?({sourceImages.length})</span></div><div className="max-h-[150px] overflow-y-auto p-2 space-y-2 custom-scrollbar">{sourceImages.map((img, idx) => (<div key={img.id} className="flex items-center bg-white p-2 rounded-lg border border-slate-100 group shadow-sm"><img src={img.src} className="w-8 h-8 object-cover rounded bg-slate-100 mr-3 pixelated"/><div className="flex-1 min-w-0"><div className="text-xs text-slate-600 truncate font-medium">{img.name}</div></div><div className="flex items-center space-x-1"><button onClick={() => {setCropTarget(img.src); setSelection(null);}} className="p-1.5 bg-indigo-100 hover:bg-indigo-500 text-indigo-600 hover:text-white rounded transition-colors mr-1" title="手动切片"><Scissors className="w-3 h-3"/></button><div className="flex items-center space-x-0.5 opacity-60 group-hover:opacity-100 transition-opacity"><button onClick={() => moveSourceImage(idx, -1)} disabled={idx === 0} className="p-1 hover:bg-slate-200 rounded text-slate-400 hover:text-slate-600 disabled:opacity-30"><ArrowUp className="w-3 h-3"/></button><button onClick={() => moveSourceImage(idx, 1)} disabled={idx === sourceImages.length - 1} className="p-1 hover:bg-slate-200 rounded text-slate-400 hover:text-slate-600 disabled:opacity-30"><ArrowDown className="w-3 h-3"/></button><button onClick={() => setSourceImages(p => p.filter(i => i.id !== img.id))} className="p-1 hover:bg-red-100 rounded text-slate-400 hover:text-red-500 ml-1"><Trash2 className="w-3 h-3"/></button></div></div></div>))}</div></div>)}
                    <div className="bg-white/50 p-3 rounded-xl border border-indigo-100"><div className="flex justify-between items-center mb-2"><span className="text-xs font-bold text-slate-500">网格切片设置</span><label className="flex items-center space-x-2 cursor-pointer select-none group"><div onClick={() => setAutoAddToTimeline(!autoAddToTimeline)} className={`w-3 h-3 rounded border flex items-center justify-center transition-colors ${autoAddToTimeline ? 'bg-indigo-500 border-indigo-500' : 'border-slate-300 bg-white'}`}>{autoAddToTimeline && <Check className="w-2.5 h-2.5 text-white" />}</div><span className="text-[10px] text-slate-400">自动加入序列</span></label></div><div className="grid grid-cols-2 gap-3 mb-3"><div><label className="text-[10px] text-slate-400 mb-1 block">�?(Cols)</label><input type="number" value={cols} onChange={(e) => setCols(Math.max(1, Number(e.target.value)))} className="w-full bg-white border border-slate-200 rounded px-2 py-1 text-xs focus:border-indigo-500 outline-none"/></div><div><label className="text-[10px] text-slate-400 mb-1 block">�?(Rows)</label><input type="number" value={rows} onChange={(e) => setRows(Math.max(1, Number(e.target.value)))} className="w-full bg-white border border-slate-200 rounded px-2 py-1 text-xs focus:border-indigo-500 outline-none"/></div></div><button onClick={handleBatchSlice} disabled={sourceImages.length === 0 || isSlicing} className="w-full py-2 bg-indigo-500 hover:bg-indigo-600 text-white disabled:opacity-50 text-xs font-bold rounded-lg flex items-center justify-center transition-all shadow-md shadow-indigo-200">{isSlicing ? <><RefreshCw className="w-3 h-3 mr-2 animate-spin" /> 切割�?..</> : <><Grid className="w-3 h-3 mr-2" /> 自动网格切割</>}</button></div>
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
            ...
            <div className="bg-white/70 backdrop-blur-md border border-white/50 rounded-3xl overflow-hidden shadow-sm flex flex-col flex-1">
                 <div className="p-4 border-b border-slate-100 bg-white/40 flex justify-between items-center"><h2 className="text-sm font-bold text-slate-500 flex items-center"><MoveRight className="w-4 h-4 mr-2 text-green-500" /> 时间�?<span className="ml-2 text-[10px] bg-white px-2 py-0.5 rounded-full border border-slate-100 text-slate-400">{timeline.length} �?/span></h2><button onClick={() => setTimeline([])} disabled={timeline.length === 0} className="text-xs text-red-400 hover:text-red-500 px-2 py-1 rounded hover:bg-red-50 transition-colors disabled:opacity-50 font-bold"><Trash2 className="w-3 h-3 inline mr-1" /> 清空</button></div>
...
const SubmissionModal = ({ onClose, commonTags = [], mode = 'create', initialData = null }) => {
  ...
  // 初始化表单数�?  useEffect(() => {
      if (initialData) {
          if (mode === 'edit') {
              ...
              // 变体模式：预填标�?只读)、标签、保留父级图�?逻辑上在后端处理，这里仅展示或允许新�?
              // 注意：用户说"除了名字和之前的图片不能修改之外都可以修改，也可以新增图�?
              // 因此这里我们加载原图，允许添加新�?              setFormData({
                  title: initialData.title + " (变体)", // 稍后会被强制覆盖或作为参�?                  content: "", // 内容清空，等待填入变体内�?                  images: initialData.images || (initialData.image ? [initialData.image] : []),
                  tags: Array.isArray(initialData.tags) ? initialData.tags : [],
                  contributor: ''
              });
          }
  ...
  const removeImage = (idx) => { 
      ...
      // 这里简化处理：允许用户在界面上删，但在后端逻辑中，变体�?append �?similar，并不直接改原图
      // 但用户说"之前的图片不能修�?，所以这里应该做个限�?      if (mode === 'variant' && initialData && idx < (initialData.images?.length || 0)) {
          alert("变体模式下，原图不可删除");
          return;
      }
  ...
  const modalTitle = mode === 'variant' ? '新增变体投稿' : mode === 'edit' ? '修改原投�? : '投稿提示�?;
...
const PromptViewer = memo(({ prompt, onSubmissionAction }) => {
  ...
  // 新增：计算当前展示的投稿�?  const currentContributor = useMemo(() => {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-12">
        ...
        <span className="ml-2 text-slate-500">加载�?..</span>
      </div>
    );
  }
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {submissions.map((sub) => (
            ...
            >
              ...
              <div className="p-3">
                <h4 className="font-bold text-sm text-slate-800 truncate mb-1">{sub.title || "未命�?}</h4>
...
export default function App() {
  ...
  // 🔴 待审核弹窗状�?  const [viewingSubmission, setViewingSubmission] = useState(null);
  const [selectedSection, setSelectedSection] = useState(null);

  // 🔴 用户认证状�?  const [currentUser, setCurrentUser] = useState(null);
  ...
  // 🔴 Firebase Authentication 监听�?  useEffect(() => {
  ...
  // 🔴 打开投稿窗口的处理函�?  const openSubmissionModal = useCallback((mode = 'create', data = null) => {
  ...
  const handleApproveSubmission = useCallback((submission, sectionId) => {
    ...
    if (submission.action === 'create') {
      ...
      // 修改投稿：更新原有提示词到指定分�?      setSections(prev => prev.map(sec => {
        if (sec.id === sectionId) {
          // 先从所有分区移除旧�?          const withoutOld = prev.map(s => ({
            ...s,
  ...
  // 🔴 处理编辑投稿（保持待处理分区打开�?  const handleEditSubmission = useCallback((submission) => {
    setEditingPrompt({
  ...
  // 🔴 查找原分�?  const findOriginalSection = useCallback((targetId) => {
  ...
  // 🔴 处理批准投稿（带分区选择�?  const handleApproveWithSection = useCallback(async (submission, sectionId) => {
                       ...
                       if (p.id === data.targetId) {
                           ...
                           if (data.action === 'edit') {
                               ...
                               const newVariant = { 
                                   ...
                                   // 🔴 关键修改：保存变体的投稿�?                                   contributor: data.contributor
                               };
                               ...
                               return {
                                   ...p,
                                   id: newTimestamp, // 更新时间�?                                   images: updatedImages,
                                   similar: [...(p.similar || []), newVariant]
                               };
                           }
                       }
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] font-sans text-slate-800 relative overflow-x-hidden">
          ...
          <div className="flex items-center space-x-3">
            ...
            {!currentUser ? (
              ...
              <button onClick={handleLogout} className="relative flex items-center space-x-1 px-4 py-1.5 rounded-full text-xs font-bold transition-all border shadow-sm hover:shadow-md active:scale-95 bg-indigo-500 border-indigo-500 text-white">
                ...
                <span>{isAdmin ? '管理�? : currentUser.email}</span>
              </button>
            )}
      ...
      {/* 🔴 修复布局偏移问题：仅�?Sidebar 打开且在宽屏下才应用 marginRight */}
      <main className="max-w-7xl mx-auto px-4 py-8 pb-24 relative z-10 transition-all duration-300" style={isSidebarOpen && window.innerWidth >= 768 ? { marginRight: `${sidebarWidth}px` } : {}}>
        ...
        {currentView === 'GIF_MAKER' ? (<GifMakerModule />) : (<>
            ...
            {isAdmin && <button onClick={handleCreateSection} className="w-full py-5 border-2 border-dashed border-slate-300/50 rounded-3xl text-slate-400 hover:text-indigo-500 hover:border-indigo-300 hover:bg-indigo-50/50 flex items-center justify-center gap-2 transition-all duration-300 group mb-8"><div className="p-2 bg-white rounded-full shadow-sm group-hover:scale-110 transition-transform"><FolderPlus size={18}/></div><span className="font-medium">新建一个分�?/span></button>}
            ...
            <div className="bg-white border-b border-slate-200 p-5 flex justify-between items-start rounded-t-3xl">
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 mt-2">
                  ...
                  <span className="text-xs text-slate-500">投稿�? {viewingSubmission.contributor}</span>
                </div>
              </div>
                ...
                <div className="grid grid-cols-3 md:grid-cols-4 gap-2 max-h-[150px] overflow-y-auto custom-scrollbar p-1">
                  {sections.map(section => (
                    ...
                    >
                      ...
                      <div className="text-[10px] text-slate-500">{section.prompts.length} �?/div>
                    </button>
                  ))}
                </div>
              </div>
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in-up">
              <div className="bg-pink-50 w-full max-w-lg rounded-3xl p-6 shadow-2xl border-2 border-pink-200">
                 ...
                 <div className="text-sm font-medium text-pink-800/80 leading-relaxed space-y-2 mb-6 text-center font-traditional">
                    ...
                    <p>如閣下仍然執意決定要觀看，請閣下自行承受觀看後的後果。若有任何不適症狀，請立刻停止觀看並及時向醫師尋求幫�?/p>
                 </div>
